...
$("#form-update").on("submit", function () {
    $("[role=group-input-details]").each((index, item) => {
        console.log("each");
        details[index] = {
            size: $(item).find(`[data-size=${index}]`).val(),
            quantity: $(item).find(`[data-quantity=${index}]`).val(),
        };
    });
    let detailsInput = $(this).find("input[name=details]");
    if (detailsInput.length == 0) {
        $(this).append('<input type="hidden" name="details">');
    }
    $(this).find("input[name=details]").val(JSON.stringify(details));
});
